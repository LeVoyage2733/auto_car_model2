...
class GlobalPlanner(Node):
    def __init__(self):
        ...
        self.get_logger().info("✅ 준비 완료! 즉시 배차 모드 가동.")
        ...
        # 상태 체크 주기를 0.5초로 단축
        self.create_timer(0.5, self.check_mission_status)
    ...
    def check_mission_status(self):
        ...
        if self.is_busy and self.current_goal:
            ...
            if dist < 2.0: 
                self.get_logger().info("🏁 하차 완료! 다음 손님을 찾습니다.")
                self.is_busy = False
                self.current_goal = None
    ...
    def process_mission(self, msg):
        ...
        self.get_logger().info(f"🚀 경로 계산: ({start_wp.x:.1f},{start_wp.y:.1f}) -> ({end_wp.x:.1f},{end_wp.y:.1f})")
        ...
        try:
            ...
            path_2 = nx.astar_path(self.graph, end_node_1, end_node_2, weight='weight')
