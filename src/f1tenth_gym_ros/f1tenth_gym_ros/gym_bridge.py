...
class GymBridge(Node):
    def __init__(self):
        ...
        # [중요] 맵 경로 및 시작 위치 기본값 (1번 정류장)
        self.declare_parameter('map_path', '/home/misys/ros2_ws/src/f1tenth_gym_ros/maps/rural_map')
        ...
        self.get_logger().info(f"📍 Bridge Config: Map={map_path}, Spawn=({sx}, {sy})")
        ...
        # 5. 토픽 설정 (기본값이 있으므로 NoneType 에러 안 남)
    ...
    def _publish_odom(self, ts):
        ...
        ego_odom.pose.pose.orientation.w, ego_odom.pose.pose.orientation.x, ego_odom.pose.pose.orientation.y, ego_odom.pose.pose.orientation.z = euler.euler2quat(0., 0., self.ego_pose[2], axes='sxyz')
        ego_odom.pose.pose.orientation.w = euler.euler2quat(0., 0., self.ego_pose[2], axes='sxyz')[0]
        ...
        if self.has_opp:
            ...
            quat = euler.euler2quat(0., 0., self.opp_pose[2], axes='sxyz')
            opp_odom.pose.pose.orientation.x, opp_odom.pose.pose.orientation.y, opp_odom.pose.pose.orientation.z, opp_odom.pose.pose.orientation.w = quat[1], quat[2], quat[3], quat[0]
    ...
    def _publish_transforms(self, ts):
        ...
        ego_t.rotation.x, ego_t.rotation.y, ego_t.rotation.z, ego_t.rotation.w = quat[1], quat[2], quat[3], quat[0]
        ...
        if self.has_opp:
            ...
            quat = euler.euler2quat(0.0, 0.0, self.opp_pose[2], axes='sxyz')
            opp_t.rotation.x, opp_t.rotation.y, opp_t.rotation.z, opp_t.rotation.w = quat[1], quat[2], quat[3], quat[0]
    ...
    def _publish_wheel_transforms(self, ts):
        ...
        quat = euler.euler2quat(0., 0., self.ego_steer, axes='sxyz')
        ego_wheel_ts.transform.rotation.x, ego_wheel_ts.transform.rotation.y, ego_wheel_ts.transform.rotation.z, ego_wheel_ts.transform.rotation.w = quat[1], quat[2], quat[3], quat[0]
        ...
        if self.has_opp:
            ...
            quat = euler.euler2quat(0., 0., self.opp_steer, axes='sxyz')
            opp_wheel_ts.transform.rotation.x, opp_wheel_ts.transform.rotation.y, opp_wheel_ts.transform.rotation.z, opp_wheel_ts.transform.rotation.w = quat[1], quat[2], quat[3], quat[0]
